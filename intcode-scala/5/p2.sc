...
case class JIT(a: Op, b: Op        ) extends Insn
case class JIF(a: Op, b: Op        ) extends Insn
...
case class Proc(tape: Seq[Int]) {
  ...
  def decode = (t(ip)%100, t.slice(ip,ip+4)) match {
    ...
    case ( 5, Array(o, a,  b, _*)) => JIT(Op(o,1,a), Op(o,2,b))
    case ( 6, Array(o, a,  b, _*)) => JIF(Op(o,1,a), Op(o,2,b))
  ...
  def step: State = decode match {
    ...
    case JIT(a,b  ) => ip  = if(a.get(t) != 0) b.get(t) else ip+3       ; step
    case JIF(a,b  ) => ip  = if(a.get(t) == 0) b.get(t) else ip+3       ; step
