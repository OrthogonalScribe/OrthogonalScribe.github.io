...
val size = 100
val proc = ICP(scala.io.Source.stdin.getLines.next split(',') map {_.toLong})

case class Pos(r: Int, c: Int) {
  def inBeam = {
    val p1 = proc.clone
    p1.step; p1 stepWithInput c
    p1.step; p1 stepWithInput r
  } == ICP.Output(1)

  def south = Pos(r+1, c  )
  def east  = Pos(r  , c+1)

  def squareFit(p: Pos) = 1 + math.abs(p.r-r)
}

// find left and right edges of beam at size-th diagonal
var (le, re) = {
  val d = (0 to size) map {i => Pos(size-i,i)} filter {_.inBeam}
  (d.head, d.last)
}

// move down by one diagonal until we fit
while ((le squareFit re) < size) {
  le = if (le.south.inBeam) le.south else le.east
  re = if (re.east .inBeam) re.east  else re.south
}
println(10000*le.c + re.r)
