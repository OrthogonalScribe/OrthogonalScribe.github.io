...
case object Black extends Color { def toNum = 0  }
case object White extends Color { def toNum = 1  }
sealed trait Color              { def toNum: Num }
object       Color {
  def apply(c: Num) = c match {
    case 0 => Black
    case 1 => White
  }
}
...
sealed trait RState
case class Move (c: Color, t: Turn ) extends RState
case class Other(a: State, b: State) extends RState

def step(c: Color) =
  ({p.step; p stepWithInput c.toNum}, p.step) match {
    case (Output(c), Output(t)) => Move(Color(c), Turn(t))
    case (a, b)                 => Other(a,b)
  }

val hull = collection.mutable.Map.empty[Point,Color].withDefaultValue(Black)
var pos  = Point(0,0)
var dir  = Point(-1,0)
var done = false
...
while (!done) step(hull(pos)) match {
  case Other(_, _) => done = true
  case Move (c, t) =>
    hull(pos) = c
    dir = t match {
      case Left  => Point(-dir.c,  dir.r)
      case Right => Point( dir.c, -dir.r)
    }
    pos = Point(pos.r+dir.r, pos.c+dir.c)
    //println(s"$pos after Move($c,$t)")
}

dump

def dump = {
  val whites = hull.filter{_._2==White}.keys
  if (whites.nonEmpty) {
    val rs = whites.map{_.r}
    val cs = whites.map{_.c}
    for   (r <- rs.min to rs.max) {
      for (c <- cs.min to cs.max)
        hull(Point(r,c)) match {
          case Black => print('.')
          case White => print('#')
        }
        println
